$dantai_url = "https://www.luminouskobe.co.jp/SSL/reservation/party.html";
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
        <tr>
          ...
          <td class="subnavi-bg-b"><a href="javascript:void(0)" onClick="MM_openBrWindow('http://www.luminouskobe.co.jp/qanda/index.html','','scrollbars=yes,resizable=yes,width=630,height=600')">よくあるご質問</a></td>
        </tr>
    </table></td>
  </tr>
...
<li><a href="https://www.luminouskobe.co.jp/SSL/mail/index.html">&gt; メール会員</a></li>
...
<li><a href="https://www.luminouskobe.co.jp/SSL/contact/index.html">&gt; お問い合わせ</a></li>
...
<ul>
<li id="F-doc"><a href="https://www.luminouskobe.co.jp/SSL/request/index.html">資料請求</a></li>
...
<td>「×」マークの日付は予約を受け付けておりません。(予約終了・貸切・全席満席・運休等)</td>
</tr>
<tr valign="top">
<td>※<br>
</td>
...
#------------------------------------------
	...
	($item_no,$item_name,$item_type,$item_bin[1],$item_bin[2],$item_bin[3],$item_bin[4],$item_bin[5],$item_kikan,$item_start_date,$item_end_date,$item_ninzu,$item_kakaku_setumei,$item_kakaku_type,$item_kakaku,$item_t_kakaku,$item_sname1,$item_skakaku1,$item_sname2,$item_skakaku2,$item_sname3,$item_skakaku3,$item_sname4,$item_skakaku4,$item_sname5,$item_skakaku5,$item_okosama,$item_mado) = split(/\,/,$item);
...
for ($flg = 1; $flg <= 5; $flg++) {
	...
	if($form{'no'} ne ""){
		...
		#窓側設定料＝込の場合で、窓側席数が残っていない場合は表示させない
		if($item_mado eq "1"){
			if($zaseki_mado{$flg} eq "" || $zaseki_mado{$flg} < 1){$no_print_flg = 1;}
		}
	}
...
if($no_print_flg eq "1"){
	print "×";
}
else{
	print "<input name=\"type\" type=\"radio\" value=\"$ngp\_$weekday\_$syuku\_$daytype\_$flg\_$j_course_no{$name[$flg]}\_$j_name{$name[$flg]}コース\_$out[$flg]\">\n";
	$gyosu_flg++;
}
...
if($gyosu_flg ne ""){
	print "<input type=\"submit\" name=\"Submit\" value=\"　次　へ　\">\n";
}
...
while(<FILE>){
	...
	$item{$no} = "$no,$name,$type,$bin1,$bin2,$kikan,$start_date,$end_date,$ninzu,$kakaku_setumei,$kakaku_type,$kakaku,$t_kakaku,$sname1,$skakaku1,$sname2,$skakaku2,$sname3,$skakaku3,$sname4,$skakaku4,$sname5,$skakaku5,$okosama,$mado,$mado_kakaku01,$jousen,$siharai,$ninzu01,$ninzu02,$option,$moption,$option2,$option2_name,$option2_setumei,$option2_ans,$naiyou,$tokuten,$bikou,$xday,$grade_up,$syouhinmei_bikou,$kokai_date,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$res,$soryo_type,$soryo,$msoryo,$res,$res,$res,$res,$modify_date,$entry_date,$endmark";
	if($ninzu eq ""){$ninzu = 1;}
...
#if(($form{'event_mail'} =~ "送信可")&&($form{'event_email'} ne "")){
if($xxxxx eq "111111111111111111111"){
$mail_magazine_subject = "メール会員登録のご案内◆ルミナス神戸２";
$mail_magazine_honbun = <<"EOM";
　　★☆★☆★　メール会員登録のご案内　★☆★☆★

　ルミナス神戸2ではメール会員の方へ、お得な情報や各種イベント、新メニュー、
メール会員様限定プラン等の情報をお届けしております。
もちろん入会費・年会費は無料です。ぜひこの機会にご登録くださいませ！

登録手順かんたん3ステップ！
１）このメールを“返信”してください(会員登録アドレスlk\@lin.acへメールを送信します)。
２）すぐにアンケートのメールが返信されます。
３）アンケートに入力・送信いただくと登録完了です！

※登録時のご注意
空メール送信後、３分以上たって返信メールがこない場合は
メール指定・ドメイン指定をされている場合がございます。
master\@luminouskobe.co.jpを受信できるように設定を変更して、再度送信して下さい。
...
&jcode'convert(*mail_magazine_subject , 'jis');
&jcode'convert(*mail_magazine_honbun , 'jis');

if (!open(MAIL,"|$sendmail $form{'event_email'}")) { print"<br><br>メール送信ができません<br><br>"; }
print MAIL "Content-type: text/plain; charset=ISO-2022-JP\n";
print MAIL "Content-Transfer-Encoding: 7bit\n";
print MAIL "To: $form{'event_email'}\n";
print MAIL "From: $mail_magazine\n";
print MAIL "Subject: $mail_magazine_subject\n";
print MAIL "\n";
print MAIL "$mail_magazine_honbun\n";
close(MAIL);
